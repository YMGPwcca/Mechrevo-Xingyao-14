...
| P02a | THMM source coverage — SOURCE_IMPORTED / CLOSED | Source S3 is recovered as a complete Project text extraction. Complete THMM and query bodies are included; the retained hash identifies the text export rather than original raw-library bytes. | [Thermal interfaces](thermal-performance.md), [sources](research-sources.md#project-sources) |
| P02b | Complete ACPI identity — PARTIAL / NEEDS_EVIDENCE | Source S12 records `acpidump`, `acpixtract`, `iasl -d dsdt.dat`, ACPICA `20251212`, and a DSDT header with length `0x833F` / 33,599 bytes, revision 2, OEM ID `XXXXXX`, OEM table ID `EDK2`, OEM revision `0x00040000`, compiler ID `ACPI`, compiler revision `0x00040000`. Original `dsdt.dat` bytes and their SHA-256 remain unavailable, and the capture alone does not assign a BIOS revision. | [ACPI/WMI](acpi-wmi.md), [sources](research-sources.md#project-sources) |
| P03 | Earlier SPFM mapping — SOURCE_RECOVERED / NEEDS_CONFIRMATION | Source S6 recovers the April `SPFM` map using `ECMD(0x91)` / `ECMD(0x92)` and a complete historical `dsdt.dsl` with DSDT length `0x836B`. Exact same-session firmware association remains unresolved; the April map must not be merged with the September `0x94` / `0x95` map. | [Thermal interfaces](thermal-performance.md), [sources](research-sources.md#project-sources) |
| P04a | Live GFNS/H2RAM correlation — SOURCE_RECOVERED / CLOSED | Source S13 contains a direct H2RAM read at `0xFEEC233B..0xFEEC233E` (`91 0f cd 0e`) followed by live WMI GFNS calls. Selector 0 returned result bytes `99 0f`; selector 1 returned `e1 0e`, each with method status `0x00`. The nearby reads differ slightly because they were sequential, but they establish live correlation between GFNS and the two dynamic FNS fields. No independent RPM calibration is established. | [Thermal interfaces](thermal-performance.md), [sources](research-sources.md#project-sources) |
...
| P13 | Successful SREP configuration — PHOTO_RECOVERED / NEEDS_CONFIRMATION | Source S14 includes the recovered post-reveal BIOS photograph, which visibly contains the revealed Boot page. Two candidate Quiet-Boot configs are also located, but the exact patcher build, final configuration used and session-to-photo association remain unproven. | [Runtime visibility](srep-runtime-reveal.md), [sources](research-sources.md#project-sources) |
...
| P15a | Device/audio enumeration — SOURCE_IMPORTED / CLOSED | Source S11 records V4L2 entries, digital/stereo microphones, ALSA/PCI bindings, loaded modules, PipeWire server `1.6.7` and a bounded `xingyao.fw` log observation. This does not establish physical camera/speaker topology or patch contents. | [Audio](audio.md), [Linux](linux.md), [sources](research-sources.md#project-sources) |
| P15b | Remaining platform inventory — PARTIAL / NEEDS_EVIDENCE | An exact-machine boot capture identifies the installed storage device as `YMTC PC41Q-1TB-B`. Panel model/EDID/refresh and adapter identity/rating remain unrecorded. | [Hardware](hardware-platform.md), [Linux](linux.md) |
...
| P16 | Current raw ROM and EC source objects — SOURCE_LOCATED / REHASH_PENDING | The private source objects `P916F-STX-current-ROM.bin` (33,554,432 bytes) and `P916F-IT5571-EC-1.09.bin` (131,072 bytes) are located. Raw-byte materialization was unavailable in this audit, so historical SHA-256 values were not independently recomputed. They are retained privately, not publicly distributed. | [Artifacts](research-artifacts.md), [sources](research-sources.md#project-sources) |
| P17a | `P916F-charge-reverse.tar.gz` parent — SOURCE_LOCATED / REHASH_PENDING | The private parent bundle is located at 10,092,171 bytes. Raw-byte materialization was unavailable in this audit, so the bundle SHA-256 remains unretained; child hashes must not substitute for it. | [Artifacts](research-artifacts.md), [sources](research-sources.md#project-sources) |
