...
- **Battery charge control:** the recorded PMC2 experiment at I/O ports `0x68`/`0x6C` established an enabled `T1=80%`, `T2=100%` configuration that stopped charging near the displayed 79–80% boundary and survived a normal reboot. A later live observation after battery depletion caused complete system power loss found `Enabled=0`, `T1=0`, `T2=0` on the next powered session; this establishes clearing for that recorded event, not every possible G3, battery-disconnect or EC-reset condition. Arbitrary threshold pairs and the exact T2 semantics remain unresolved. [Protocol](docs/battery-charge-limit.md) · [Raw validation](docs/validation.md) · [Power-loss observation](docs/battery-limit-power-loss-observation.md).
